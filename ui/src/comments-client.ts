...
export class CommentsClient extends ZomeClient<CommentsSignal> {
	...
	async getAllRevisionsForComment(
		commentHash: ActionHash,
	): Promise<Array<EntryRecord<Comment>>> {
		const records: Record[] = await this.callZome(
			'get_all_revisions_for_comment',
			commentHash,
		);
	...
	async getCommentsForCommented(commented: AnyDhtHash): Promise<Array<Link>> {
		return this.callZome('get_comments_for_commented', commented);
	}

	async getDeletedCommentsForCommented(
		commented: AnyDhtHash,
	): Promise<
		Array<[SignedActionHashed<CreateLink>, SignedActionHashed<DeleteLink>[]]>
	> {
		return this.callZome('get_deleted_comments_for_commented', commented);
	}
}
