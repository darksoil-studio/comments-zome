...
export class CommentsContext extends LitElement {
  ...
  connectedCallback() {
    ...
    if (!this.role) {
      throw new Error(`<comments-context> must have a role="YOUR_DNA_ROLE" property, eg: <comments-context role="role1">`);
    }
    if (!this.client) {
    ...
    this.store = new CommentsStore(new CommentsClient(this.client, this.role, this.zome));
  }
