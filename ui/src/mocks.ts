...
export class CommentsZomeMock extends ZomeMock implements AppClient {
  ...
  async create_comment(comment: Comment): Promise<Record> {
    ...
    if (comment.reply_to) {
      const existingReplyTo = this.commentsForComment.get(comment.reply_to) || [];
      this.commentsForComment.set(comment.reply_to, [...existingReplyTo, {
  ...
  async update_comment(
    input: { original_comment_hash: ActionHash; previous_comment_hash: ActionHash; updated_comment: Comment },
  ): Promise<Record> {
    const record = await fakeRecord(
      await fakeUpdateEntry(input.previous_comment_hash, undefined, undefined, fakeEntry(input.updated_comment)),
  ...
  async get_comments_for_comment(commentHash: ActionHash): Promise<Array<Link>> {
    return this.commentsForComment.get(commentHash) || [];
  }
}

export async function sampleComment(client: CommentsClient, partialComment: Partial<Comment> = {}): Promise<Comment> {
  return {
    ...{
